...
export type DiagramSlide = {
    ...
    connectionLines?: { from: string; to: string }[];
...
export type KpiSlide = {
    ...
    items: { label: string; value: string; change: string; status: 'good' | 'bad' | 'neutral' }[];
...
export type FlowChartSlide = {
    ...
    startNodeShape?: 'circle' | 'rect';

};



export type StepUpSlide = {
...
export type GanttChartSlide = {
    ...
    header: { groups: string[]; periods: number[]; totalPeriods: number };

    rows: { taskName: string; bars: { startIndex: number; span: number; color: string }[] }[];
...
export type SlideData = SlideNode[];
